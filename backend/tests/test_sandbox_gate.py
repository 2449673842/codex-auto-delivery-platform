...
class TestSandboxGateUnit:
    ...
    async def test_blocked_not_applied(self, client, task, run, db_session):
        ...
        data = (await client.get(f"/api/tasks/{task['id']}/sandbox/gate")).json()["data"]
        assert "sandbox_not_applied" in [r["reason"] for r in data["blocked_reasons"]]

    # ── blocked: no changed files ──
    @pytest.mark.asyncio
    async def test_blocked_no_changed_files(self, client, task, run, db_session):
        ...
        data = (await client.get(f"/api/tasks/{task['id']}/sandbox/gate")).json()["data"]
        assert "no_changed_files" in [r["reason"] for r in data["blocked_reasons"]]
    ...
    async def test_blocked_forbidden_path(self, client, task, run, db_session):
        ...
        data = (await client.get(f"/api/tasks/{task['id']}/sandbox/gate")).json()["data"]
        assert "forbidden_path" in [r["reason"] for r in data["blocked_reasons"]]
