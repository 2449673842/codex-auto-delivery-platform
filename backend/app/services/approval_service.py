...
async def evaluate_approval(
    ...
    risk_result = risk_assessment_service.assess_risk(
        diff=diff, raw_result_json=raw_json, confidence_score=confidence,
        tests_passed=data.tests_passed, sonar_passed=data.sonar_passed,
        security_issues_found=data.security_issues_found, changed_files=data.changed_files,
    )
    ...
    return ApprovalEvaluationResponse(
        id=decision.id, task_id=task_id,
        risk_level=risk_level, risk_reasons=risk_result["risk_reasons"],
        auto_approve_allowed=auto_approve_allowed, human_required=human_required,
        decision_reason=policy_snapshot["name"], blocked_reasons=blocked_reasons,
        tests_passed=data.tests_passed, security_issues_found=data.security_issues_found,
    )
...
async def auto_approve(
    ...
    if task.status != "reviewing":
        raise HTTPException(status_code=409, detail=f"Cannot auto-approve task in status '{task.status}', only 'reviewing' allowed")
    ...
    body = TaskStatusTransition(actor=actor, message=message or "Auto-approved")
